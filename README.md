...
| Chat 追问对话 |
| ------------- |
| 在翻译或识别结果面板点击「Chat」按钮，即可进入多轮对话窗口。新建翻译对话时会自动解释原文与所选服务的译文，并在后续追问中保留上下文。OCR 对话有图时会携带原图；配置支持图片输入的模型后，即使没有识别出文字也能发起追问。对话仍支持编辑、重新生成和自定义系统提示词。 |
...
-   [x] 一键复制翻译、OCR、语音合成及收藏服务实例，保留 API 地址和密钥等配置
-   [x] 临时添加翻译服务对比结果，不改变已保存的启用或暂停状态；临时 OCR 服务菜单支持关键词筛选
-   [x] 增大多个 OCR 结果的阅读区域；翻译结果生成时不再自动向下滚动
